...
def WF(s,mu,N, G, seed = 0):
    ...
    p[0] = np.array([1,0]) # initial frequency of the mutant allele
...
def WF_FD(s,mu,N, G, seed = 0):
    ...
    for i in range(G): # iterating over generations
        w = 1+s*(1-p[i,1]) # fitness of the mutant allele
        S = np.array([[1, 0], [0, w]]) # selection matrix
        E = S@M # evolutionary matrix
        p[i+1] = E@p[i] # updating the frequency of the mutant allele
        p[i+1] /= np.sum(p[i+1]) # normalizing the frequency
...
def WF_bottleneck(s,mu,N, G, bottleneck, seed = 0):
    # s: selection coefficient
    # mu: mutation rate
    # N: population size
    # G: number of generations
...
def WF_DFE(s,mu,N, G, seed = 0):
    ...
    u, sigma = s, s/5
    s_ = stats.truncnorm(
        (lower - u) / sigma, (upper - u) / sigma, loc=u, scale=sigma).rvs(G)
